...
microk8s.kubectl create namespace $MR3_NAMESPACE
...
if [ $RUN_AWS_EKS = true ]; then
  ...
  microk8s.kubectl create -n $MR3_NAMESPACE -f $YAML_DIR/workdir-pvc.yaml 
fi

microk8s.kubectl create -n $MR3_NAMESPACE secret generic env-secret --from-file=$BASE_DIR/env.sh
microk8s.kubectl create -n $MR3_NAMESPACE configmap $CONF_DIR_CONFIGMAP --from-file=$BASE_DIR/conf/

if [ $CREATE_KEYTAB_SECRET = true ]; then
  microk8s.kubectl create -n $MR3_NAMESPACE secret generic $KEYTAB_SECRET --from-file=$BASE_DIR/key/
else 
  microk8s.kubectl create -n $MR3_NAMESPACE secret generic $KEYTAB_SECRET
fi

if [ $CREATE_WORKER_SECRET = true ]; then
  microk8s.kubectl create -n $MR3_NAMESPACE secret generic $WORKER_SECRET --from-file=$WORKER_SECRET_DIR
else 
  microk8s.kubectl create -n $MR3_NAMESPACE secret generic $WORKER_SECRET
fi

microk8s.kubectl create -f $YAML_DIR/cluster-role.yaml
microk8s.kubectl create -f $YAML_DIR/hive-role.yaml
...
if [ $CREATE_SERVICE_ACCOUNTS = true ]; then
  microk8s.kubectl create -f $YAML_DIR/hive-service-account.yaml
...
microk8s.kubectl create clusterrolebinding hive-clusterrole-binding --clusterrole=node-reader --serviceaccount=$MR3_NAMESPACE:$MR3_SERVICE_ACCOUNT
microk8s.kubectl create rolebinding hive-role-binding --role=hive-role --serviceaccount=$MR3_NAMESPACE:$MR3_SERVICE_ACCOUNT -n $MR3_NAMESPACE

microk8s.kubectl create clusterrolebinding master-clusterrole-binding --clusterrole=node-reader --serviceaccount=$MR3_NAMESPACE:$MASTER_SERVICE_ACCOUNT
microk8s.kubectl create rolebinding master-role-binding --role=master-role --serviceaccount=$MR3_NAMESPACE:$MASTER_SERVICE_ACCOUNT -n $MR3_NAMESPACE

microk8s.kubectl create rolebinding worker-role-binding --role=worker-role --serviceaccount=$MR3_NAMESPACE:$WORKER_SERVICE_ACCOUNT -n $MR3_NAMESPACE
...
microk8s.kubectl create -n $MR3_NAMESPACE configmap client-am-config \
...
if [ $CREATE_PROMETHEUS_SERVICE = true ]; then
  microk8s.kubectl create -f $YAML_DIR/prometheus-service.yaml
fi

microk8s.kubectl create -f $YAML_DIR/hive.yaml
microk8s.kubectl create -f $YAML_DIR/hiveserver2-service.yaml
